...
Arduino IDE project — there is no CLI build script in the repo. Open `jellybox-firmware.ino`, select **ESP32 Dev Module** and partition scheme **Minimal SPIFFS (1.9MB APP with OTA/190KB SPIFFS)** (the default 1.2 MB partition is too small for TLS + libraries; OTA also requires two app partitions, so `Huge APP` is *not* an option), then Upload. If the board stalls at "Connecting…", hold the GPIO0 BOOT button until the upload starts.
...
Single-file `.ino` sketch driving four hardware peripherals through header-only wrappers. Each `*.h` defines one class with inline implementation — there are no `.cpp` files. The non-class header `JellyboxLogo.h` holds the 72×72 PROGMEM splash bitmap.
...
**State machine.** `AppState` in `jellybox-firmware.ino` has six states: `UNCONFIGURED` → `CONNECTING` → `BOOTSTRAPPING` → `READY` ⇄ `SCAN_MODE`, plus `UPDATING` reachable from `BOOTSTRAPPING`. Transitions are driven by:
- WiFiManager autoConnect result in `setup()`
- `BootstrapResult` from `GET /api/device/me` every `BOOTSTRAP_INTERVAL_MS` (30 s) — a `401` drops back to `UNCONFIGURED` and shows the unpaired screen; `scanMode: true` in the response puts the device into `SCAN_MODE`; a `latestFirmware` block whose version differs from the compiled-in `FIRMWARE_VERSION` enters `UPDATING`.
...
**OTA flow.** `OTAUpdater.h` owns the download / install / rollback-commit logic. The server is the gating authority — `latestFirmware` is only present in `/api/device/me` when the user has flagged the device for an update in the dashboard, so devices never auto-flash. Freshly written images are marked `PENDING_VERIFY`; once the new firmware reaches the server successfully it calls `markCurrentImageValid()` (logged as `[OTA] Marking current image valid`). If it crashes or watchdog-resets first, the bootloader rolls back. Local builds compile with `FIRMWARE_VERSION="dev"` and short-circuit the OTA check so a USB-flashed dev build never overwrites itself with a release.
